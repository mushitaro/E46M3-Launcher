...
**2026-08-09、WiFi を切った状態でランチャーの M コンソールから TUNER を起動し、完全に描画された。** フォント・スタイル・ダイアログすべてキャッシュから。エラーページでも白画面でもない。

WiFi を落とすと ADB 自身が切れるため、テストは切り離したスクリプトとして端末側に渡し、復帰後にスクリーンショットを回収した（`tools/deploy.sh --offline`）。

**skipWaiting が無いことの実務上の帰結**：プライム（オンラインで1回開く）だけでは足りず、**閉じて開き直すまで新ビルドは有効化されない**。deploy.sh はその2段目を含む。
...
### 10.13 再起動をまたいだ復帰（ランチャー側）

データログを終えてキーオフ→オンで**ヘッドユニットがコールドブートする** **[V]** ため、TUNER が初期画面で立ち上がる。データは IndexedDB に残るので失われるのは**居場所だけ**だが、戻る道が復旧導線しかないのは保険を毎回通ることになる。

**分担**：再起動後に前面へ戻せるのは**ランチャーだけ**（死んだ Web アプリは自分を起動できない）。どのセッションを開くかは **TUNER だけ**が知っている。ランチャーが運ぶのは「これは復帰である」という事実ひとつ＝ **`?resume=1`**。TUNER 側の指示書は `docs/05-tuner-resume-spec.md`。

#### ★ フラグだけでは事故る — 起動直後の窓が要る

「前面のまま終了した」は**2つの別事象**で真になる：

1. 電源が落ちた → 開き直すのが正しい
2. **TUNER 表示中にこのランチャーが LMK で殺された** → 開き直すのは**間違い**。この場合に次にプロセスを起こすのは「TUNER から出るために HOME を押した」であり、押した瞬間 TUNER に弾き返される

空きメモリ1.1GB・heap上限192MB のこの端末で 2 は仮定ではない。区別は `SystemClock.elapsedRealtime()`：ブートなら3分以内にランチャーが画面に出る。窓は**緩め**に取った — 短すぎる場合の失敗は「黙って何もしない」で、誰も報告できない。

#### 驚かせない形

```
コンソールが普通に出る
  → LCD の通知行に  TUNER IN 3 / 2 / 1
  → 画面のどこかに触れたら中止（onUserInteraction）
  → 触られなければ TUNER を全画面で開く
```

キーを回してラジオを触りたいだけの時に全画面の解析ツールが出るのは事故なので、可視のカウントダウンと**最も広いキャンセル網**（`onUserInteraction` はタッチもキーも、コンソールが何を押されたか判断する前に拾う）を置いた。3秒は行儀だけでなく **Chrome セッションの暖機**でもある — 温まっていないと TWA ではなくツールバー付き Custom Tab で戻ってしまう。

**記録は起動を試みる前に消す。** TUNER が起動時に落ちるようになっても、次のキーサイクルは素のコンソールに戻る。

#### 検証（エミュレータ）**[V]**

| | |
|---|---|
| カウントダウン表示 | `TUNER IN 3` が予約済み通知行に。レイアウト移動なし |
| 触れる → 中止 | 4秒後もフォーカスはランチャー |
| 放置 → 起動 | `TargetLauncher: opening …/?resume=1` |
| 手押し（M → TUNER） | `TargetLauncher: opening …/`（パラメータ無し） |

URI は ActivityManager の START 行では切られるため、**自前でログを出すようにした**。実車で復帰経路が効いたかを見る手段でもある。

#### 未確認 [U]

**バックカメラより優先されないこと。** 始動直後にリバースへ入れた時、TUNER が前面でも後退画像が出なければならない。カメラは `backcar_daemon` + MainUI 側で独立 **[V]** なので上書きされる**はず**だが、実測していない。**車庫で必ず試す。**
